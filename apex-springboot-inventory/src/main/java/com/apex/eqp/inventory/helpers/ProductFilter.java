...
public class ProductFilter {
    ...
    public List<Product> removeRecalledFrom(Collection<Product> allProduct) {
        return allProduct.stream().filter(product -> filterByName(product, recalledProducts)).collect(Collectors.toList());
    }

    private static boolean filterByName(Product product,Set<String> recalledProducts) {
        return product.getName() != null && !recalledProducts.contains(product.getName());
